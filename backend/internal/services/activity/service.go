...
// AnalyzeIncoming, contact-sent mesaj geldiğinde çağrılır. Signature channel.IncomingHook ile aynı.
//
// 2026-05-16 (Katman 2 outbox): Bu fonksiyon ARTIK direkt AI çağırmıyor. Mesajı analysis_queue'ya
// yazıyor; bir background worker (worker.go) kuyruğu işliyor. Bu sayede:
//   - Handler 5-10 ms'de döner (önceden 1-25 saniye olabiliyordu)
//   - Crash/restart sonrası kuyruktaki kayıtlar boot'tan sonra işlenir → "kayıp" yok
//   - Haiku rate-limit / timeout → exponential backoff retry, sessiz kayıp yok
//   - Coverage SQL ile cevaplanabilir (analysis_queue.status, analysis_attempts metrikleri)
//
// Kullanılmayan parametreler (customerID, channel, text): contract korunsun diye burada;
// worker.go message_id'den DB'den çekiyor (her zaman taze).
func (s *Service) AnalyzeIncoming(orgID int64, customerID *int64, messageID int64, channel, text string) {
	if messageID <= 0 || orgID <= 0 {
		return
	}
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	if err := s.Enqueue(ctx, orgID, messageID); err != nil {
		log.Printf("activity: enqueue failed (org=%d msg=%d): %v", orgID, messageID, err)
	}
	// customerID/channel/text artık worker'da DB'den taze çekiliyor; bu parametreler imza
